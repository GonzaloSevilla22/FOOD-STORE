    ...
    <div className="space-y-5">
            ...
            {usuarios.map((user) => (
              <tr key={user.id}>
                ...
                <td className="px-4 py-3">
                  <span className="text-xs text-slate-400 dark:text-gray-300">—</span>
                </td>
                <td className="px-4 py-3">
        ...
        <div className="space-y-3 rounded-xl border border-orange-100 dark:border-gray-500 bg-white dark:bg-gray-800 p-4 shadow-sm">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <label className="grid gap-1 text-sm text-slate-700 dark:text-gray-300">
              Nombre
              <input type="text" value={form.nombre} onChange={(event) => setForm((prev) => (prev ? { ...prev, nombre: event.target.value } : prev))} className="rounded border border-orange-200 dark:border-gray-500 px-3 py-2" />
            </label>
            <label className="grid gap-1 text-sm text-slate-700 dark:text-gray-300">
              Apellido
              <input type="text" value={form.apellido} onChange={(event) => setForm((prev) => (prev ? { ...prev, apellido: event.target.value } : prev))} className="rounded border border-orange-200 dark:border-gray-500 px-3 py-2" />
            </label>
            <label className="grid gap-1 text-sm text-slate-700 dark:text-gray-300 md:col-span-2">
              Celular
              <input type="text" value={form.celular} onChange={(event) => setForm((prev) => (prev ? { ...prev, celular: event.target.value } : prev))} className="rounded border border-orange-200 dark:border-gray-500 px-3 py-2" />
            </label>
      ...
      <Modal isOpen={showCreateModal} title="Crear usuario" onClose={() => setShowCreateModal(false)}>
        <div className="grid gap-3">
          <label className="grid gap-1 text-sm text-slate-700 dark:text-gray-300">
            Nombre
            <input type="text" value={createForm.nombre} onChange={(e) => setCreateForm((p) => ({ ...p, nombre: e.target.value }))} className="rounded border border-orange-200 dark:border-gray-500 px-3 py-2" />
          </label>
          <label className="grid gap-1 text-sm text-slate-700 dark:text-gray-300">
            Apellido
            <input type="text" value={createForm.apellido} onChange={(e) => setCreateForm((p) => ({ ...p, apellido: e.target.value }))} className="rounded border border-orange-200 dark:border-gray-500 px-3 py-2" />
          </label>
          <label className="grid gap-1 text-sm text-slate-700 dark:text-gray-300">
            Email
            <input type="email" value={createForm.email} onChange={(e) => setCreateForm((p) => ({ ...p, email: e.target.value }))} className="rounded border border-orange-200 dark:border-gray-500 px-3 py-2" />
          </label>
          <label className="grid gap-1 text-sm text-slate-700 dark:text-gray-300">
            Celular (opcional)
            <input type="text" value={createForm.celular} onChange={(e) => setCreateForm((p) => ({ ...p, celular: e.target.value }))} className="rounded border border-orange-200 dark:border-gray-500 px-3 py-2" />
          </label>
          <label className="grid gap-1 text-sm text-slate-700 dark:text-gray-300">
            Contraseña
            <input type="password" value={createForm.password} onChange={(e) => setCreateForm((p) => ({ ...p, password: e.target.value }))} className="rounded border border-orange-200 dark:border-gray-500 px-3 py-2" />
          </label>
          <button
            type="button"
